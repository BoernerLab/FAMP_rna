...
class Ensemble:
    ...
    def __init__(self, directory, parameters, compute_anisotropy=False, verbose=True, units="A"):
        ...
        for i in range(len(ps["name"])): # Liste der Namen sollten durch die xtc oder PDB Dateien entstehen bzw. Lister der eingeladenen Dateien
            filelist_rkappa = sorted(glob.glob(os.path.join(directory, ps["unix_pattern_rkappa"][i]))) # Dateiliste der rkappa Dateien --> Ersetzen
            if not filelist_rkappa:
                raise IndexError( # f Strings verwenden
            ...
            if compute_anisotropy:
                filelist_don_coords = sorted(glob.glob(os.path.join(directory, ps["unix_pattern_don_coords"][i]))) # Koordinaten Donor und Acceptor mit MD Traj einlesen
                ...
                if (not filelist_don_coords) or (not filelist_acc_coords):
                    ...
                    if verbose:
                        print("Loading files...")
                    self.species.append( ######################## Hier wird eine Spezies dann hinzugefügt innerhalb der Schleife für jeden Namen in der Liste
                        Species(
                            ps["name"][i],
                            ps["probability"][i],
                            filelist_rkappa,
...
class Experiment:
    ...
    def __init__(
        ...
        if verbose:
            self.print_settings(compute_anisotropy) # Funktion zum Ausgeben von Parametern und anderen Stuff
