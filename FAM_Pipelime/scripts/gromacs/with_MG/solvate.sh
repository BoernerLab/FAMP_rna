...
Usage: solvate.sh -f <structure file (.gro, .pdb)> -w <solvent file (tip4p.gro, spc216.gro)> -d <mdp directory> -b <distance to box edge (default: 1nm)> -i <concentration of divalent ions in mol/l>" 1>&2; exit 1; }
...
gmx pdb2gmx -f "$structureFile" -o em/"$structureName".gro -p "$structureName".top -i em/"$structureName".itp -missing -ignh|| { echo "-> Error: gmx pdb2gmx failed" ; cleanup em; exit 1; }
...
gmx genion -s em/"$structureName".tpr -o em/"$structureName".gro -p "$structureName".top -nname Cl -pname K -conc 0.1 || { echo "-> Error: gmx genion failed" ; cleanup em; exit 1; }

gmx grompp -f "$mdp_dir"/em.mdp -c em/"$structureName".gro -p "$structureName".top -o em/"$structureName".tpr -po em/"$structureName".mdp -maxwarn 2 || { echo "-> Error: grompp after solvation failed" ; cleanup em; exit 1; }


gmx genion -s em/"$structureName".tpr -o em/"$structureName".gro -p "$structureName".top -nname Cl -pname MG -pq 2 -conc "$dion_conc" || { echo "-> Error: gmx genion failed" ; cleanup em; exit 1; }


gmx grompp -f "$mdp_dir"/em.mdp -c em/"$structureName".gro -p "$structureName".top -o em/"$structureName".tpr -po em/"$structureName".mdp -maxwarn 2 || { echo "-> Error: gmx grompp after genion failed" ; cleanup em; exit 1; }
